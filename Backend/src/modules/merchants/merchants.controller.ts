...
export class MerchantsController {
  ...
  @Post(':id/integrations/salla/connect')
  async connectSalla(
    @Param('id') merchantId: string,
    @Body() body: { code: string },
  ) {
    await this.svc.connectSallaIntegration(merchantId, body.code);
    return { status: 'ok' };
  }

  @Post(':id/integrations/salla/disconnect')
  async disconnectSalla(@Param('id') merchantId: string) {
    await this.svc.disconnectSallaIntegration(merchantId);
    return { status: 'ok' };
  }
