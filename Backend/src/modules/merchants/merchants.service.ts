...
export class MerchantsService {
  ...
  async connectSallaIntegration(
    merchantId: string,
    authorizationCode: string,
  ): Promise<void> {
    // 1. أرسل طلب لسلة لجلب AccessToken/RefreshToken
    const sallaAuthRes = await axios.post(
    ...
    const { access_token, refresh_token, store_id, store_url } =
      sallaAuthRes.data;

    // 2. حدّث التاجر في قاعدة البيانات
  ...
  /**
   * قطع الربط مع سلة (الغاء التكامل وحذف التوكنات)
   */
  async disconnectSallaIntegration(merchantId: string): Promise<void> {
    await this.merchantModel.findByIdAndUpdate(merchantId, {
      'integrations.salla': {
        active: false,
        accessToken: null,
    ...
    // (اختياري) أوقف الـ Scheduler/Webhook
  }

  /**
   * تحديث التوكن (Refresh Token)
   */
  async refreshSallaIntegrationToken(
    merchantId: string,
  ): Promise<string | null> {
    const merchant = await this.merchantModel.findById(merchantId).exec();
    const salla = merchant?.integrations?.salla;
    if (!salla?.refreshToken) throw new Error('No Salla refreshToken');

    // 1. أرسل طلب refresh لسلة
    const res = await axios.post<SallaTokenResponse>(
      'https://accounts.salla.sa/oauth2/token',
      {
        grant_type: 'refresh_token',
        client_id: process.env.SALLA_CLIENT_ID,
        client_secret: process.env.SALLA_CLIENT_SECRET,
        refresh_token: salla.refreshToken,
        redirect_uri: process.env.SALLA_REDIRECT_URI,
      },
    );
    ...
    await this.merchantModel.findByIdAndUpdate(merchantId, {
      'integrations.salla.accessToken': access_token,
      'integrations.salla.refreshToken': refresh_token,
      'integrations.salla.lastSync': new Date(),
    });
